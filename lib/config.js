...
var api = {
    getDisplayHostName: function(remoteHostPattern) {
        var hostname = os.hostname();

        return remoteHostPattern && new RegExp(remoteHostPattern).test(hostname) ? hostname : 'localhost';
    },
...
var Config = function(bundleName) {
    ...
    // хост web-сервера
    this._displayHostName = api.getDisplayHostName(this._data.remoteHost);
};
...
Config.prototype = {
    ...
    /**
     * Возвращает отображаемый хост для веб-сервера
     * @returns {String}
     */
    getDisplayHostName: function() {
        return this._displayHostName;
    }
};
