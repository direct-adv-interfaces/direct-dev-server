...
Config.prototype = {
    ...
    getDefaultUrl: function() {
        ...
        return target ?
            api.normalizeUrl(target.replace('?', this._bundle)) :
            undefined;
    },
    ...
    /**
     * Возвращает URL относительно хоста
     * @param {String} url - URL относительно корня проекта
     * @returns {String}
     */
    resolveUrl: function(url) {
        var baseUrl = this._baseUrl || [];
        var relativeUrl = api.normalizeUrl(url) || [];

        return '/' + [].concat(baseUrl).concat(relativeUrl).join('/');
    },
